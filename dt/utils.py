...
def get_batch_(trajectories, info, variant):
...
def get_batch(trajectories_list, info, variant, train_env_name_list):
    ...
    def fn(batch_size=per_env_batch_size, index=None):
        ...
        env_name = index
        get_batch_fn = get_batch_(trajectories_list[env_id], info[env_name], variant) 
...
""" data processing """
def load_meta_data(env_name_list, data_save_path, optimal=True):
    trajectories_list = []
...
def eval_episodes(target_rew, info, variant, env, env_name):
    ...
    def fn(model, info, prompt=None, masks=None):
        returns = []
        success = []
        length = []
