...
class DecisionTransformerGrow(nn.Module):
    ...
    def multi_task_forward(self, task_block, env_name, states, actions, rewards, returns_to_go, timesteps, 
        ...
        if len(task_block) == 1:
            action_preds = self.forward(
                task_block[0], env_name, states, actions, rewards, returns_to_go, timesteps, attention_mask=attention_mask
            )[env_ids]
        ...
        # self.eval()
        with torch.no_grad():
            for i, task_id in enumerate(task_block):
                if task_id == -1:
                    continue
                ...
                action_preds, feature = self.forward(
                    task_id, env_name, states, actions, rewards, returns_to_go, timesteps, attention_mask=attention_mask, return_features=True
                )
                ...
                if i > 0 and self.recursion:
                    ...
                    concat = torch.cat(feature_prev, dim=0)
                    res = concat * attn.permute(1, 0).unsqueeze(-1).unsqueeze(-1)
                    old_feature = torch.sum(res, dim=0)
                    action_preds = torch.cat([old_feature, feature], dim=-1)
                    action_preds = self.final_head[i](action_preds)

                action_preds_prev.append(action_preds.unsqueeze(0).detach())
                feature_prev.append(feature.unsqueeze(0).detach())
        ...
        new_action_preds, feature = self.forward(
            task_block[env_ids], env_name, states, actions, rewards, returns_to_go, timesteps, attention_mask=attention_mask, return_features=True
        )
    ...
    def multi_task_eval(self, task_block, env_name, states, actions, rewards, returns_to_go, timesteps, 
                           attention_mask=None, env_ids=None, return_features=False, is_eval=False):
        action_preds_prev = []
        feature_prev = []
        env_ids = self.env_list.index(env_name)
    ...
    def forward(self, env_id, env_name, states, actions, rewards, returns_to_go, timesteps, attention_mask=None, env_ids=None, return_features=False):
    ...
    def get_action(self, env_name, states, actions, rewards, returns_to_go, timesteps, info, masks, **kwargs):
        ...
        if masks is not None and env_id < num_head and len(self.headq) > 0:
            ...
            while id > 0:
                if masks[id] == -1:
                    ...
                    break
            action_preds = self.forward(
                id, env_name, states, actions, None, returns_to_go, timesteps, attention_mask=attention_mask)[id]
